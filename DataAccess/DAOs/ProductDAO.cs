...
namespace DataAccess.DAOs
{
    public class ProductDAO 
    {
        ...
        {
            return await _context.Products.Include(p => p.Images.Where(i => !i.Isdelete)).Include(p => p.SizeChart)
        ...
        {
            return await _context.Products.Include(p => p.Images.Where(i => !i.Isdelete))
                    .Include(p => p.SizeChart)
                        .ThenInclude(sc => sc.Images.Where(i => !i.Isdelete))
        ...
        public async Task<IEnumerable<Product>> SearchProductsInAdminAsync(string? searchTerm)
        {
            var productsQuery = _context.Products
                .Include(p => p.Images.Where(i => !i.Isdelete)).Include(p => p.SizeChart)
                .Include(p => p.Category).Include(p => p.Brand)
                .Include(p => p.Owner)
                .Where(p => !p.Isdelete);
